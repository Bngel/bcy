...
class DiscussCardHomeFragment : LinearLayout {
    ...
    constructor(context: Context, number: String, id: String, username: String?, photo: String?, cosPhoto:List<String>?, label: List<String>?, description: String?, createTime: String?):super(context) {
        ...
        date_discuss_card_HomeFragment.text = createTime?:""
        getCounts(id, number)
        setLikeStatus(context, id, number)
        setFavorStatus(context, id, number)
    }
    ...
    private fun setLikeStatus(context: Context, id: String, number: String) {
        val likeService = LikeControllerService()
        val acgJudgeLikes = likeService.getAcgJudgeLikes(id, listOf(number))
        var status = 0
        ...
        like_btn_discuss_card_HomeFragment.setOnClickListener {
            if (ConstantRepository.loginStatus) {
                if (status == 0) {
                    val postAcgLikeCos = likeService.postAcgLikeCos(id, number)
                    ...
                    val deleteAcgLikeCos = likeService.deleteAcgLikeCos(id, number)
    ...
    private fun setFavorStatus(context: Context, id: String, number: String) {
        val favorService = FavorControllerService()
        val acgJudgeFavor = favorService.postAcgJudgeFavor(id, listOf(number))
        var status = 0
        ...
        star_btn_discuss_card_HomeFragment.setOnClickListener {
            if (ConstantRepository.loginStatus) {
                if (status == 0) {
                    val postAcgFavorCos = favorService.postAcgFavorCos(id, number)
                    ...
                    val deleteAcgFavorCos = favorService.deleteAcgFavorCos(id, number)
