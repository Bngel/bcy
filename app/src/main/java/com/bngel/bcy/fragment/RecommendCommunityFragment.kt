...
import kotlinx.android.synthetic.main.fragment_recommend_community.*

class RecommendCommunityFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_recommend_community, container, false)
    }
    ...
    private fun circleEvent() {
        if (ConstantRepository.loginStatus) {
            val acgPersonalCircle =
                circleService.getEsRecommendCircle(CIRCLE_COUNT, pageNow, InfoRepository.user.id )
            if (acgPersonalCircle != null && acgPersonalCircle.msg == "success") {
                val circles = acgPersonalCircle.data.recommendCircleList
                cards_recommendCommunityFragment.removeAllViews()
                for (circle in circles) {
                    val view = CommunityCardView(parentContext!!, circle.photo, circle.circleName, circle.description, 0, detailLauncher)
                    cards_recommendCommunityFragment.addView(view)
                }
            }
        }
    }
